...
document.getElementById("classify").addEventListener("click", async () => {
  ...
  for (const c of companies) {
    // Combina descrierea si tag-urile pentru textul de comparat
    const text = `${c.description || ""} ${c.business_tags || ""}`;
    const words = cleanText(text);

    let bestMatches = [];

    // Compar compania cu fiecare eticheta din taxonomie
    ...
    // Sortez rezultatele dupa scor (descrescator)
    bestMatches.sort((a, b) => b.score - a.score);

    // Pastrez cele mai bune 3 potriviri
    const top = bestMatches.slice(0, 3).map(x => x.label).join(", ");

    // Salvez rezultatul pentru aceasta companie
    results.push({
  ...
  // Afisez rezultatele finale in format JSON in <pre> din pagina HTML
