...
// Nu foloseste "fs" (care merge doar in Node.js)
async function parseCSV(file) {
  const text = await file.text(); // citim tot continutul fisierului

  // Curatam fisierul (eliminam BOM si spatiile inutile)
  const clean = text.trim().replace(/^\uFEFF/, "");

  // Impartim pe linii
  const lines = clean.split(/\r?\n/);

  // Detectam separatorul (poate fi "," sau ";")
  const firstLine = lines[0];
  const separator = firstLine.includes(";") ? ";" : ",";

  // Extragem anteturile (coloanele)
  const headers = firstLine.split(separator).map(h => h.trim().replace(/['"]+/g, ""));

  const rows = [];

  // Trecem prin fiecare linie (incepand cu a doua)
  for (let i = 1; i < lines.length; i++) {
    const values = lines[i].split(separator).map(v => v.trim().replace(/['"]+/g, ""));
    const obj = {};

    // Cream un obiect cu perechi "coloana: valoare"
    headers.forEach((h, j) => {
  ...
  return rows; // returnam lista de obiecte
}

// Cand apasam pe butonul "Classify", executam codul
document.getElementById("classify").addEventListener("click", async () => {
  ...
  // Verificam daca ambele fisiere au fost incarcate
  if (!companyFile || !taxonomyFile) return alert("Incarca ambele fisiere CSV!");

  // Citim si parsăm fisierele CSV
  const companies = await parseCSV(companyFile);
  const taxonomy = await parseCSV(taxonomyFile);

  console.log("✅ Parsed taxonomy:", taxonomy.slice(0, 5)); // Afisam primele 5 pentru test

  const results = [];

  // Parcurgem fiecare companie
